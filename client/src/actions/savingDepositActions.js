...
const apiUrl = "/api/";
export const toggleAddSavingDeposit = () => {
  return {
    type: 'TOGGLE_ADD_SAVING_DEPOSIT'
  }
}
export const addNewSavingDeposit = (savingDeposit) => {console.log(savingDeposit)
  return (dispatch) => {
    ...
    return fetch(apiUrl, {
      ...
      if(response.ok){
        response.json().then(data => {console.log(data.savingDeposit);
          dispatch(addNewSavingDepositRequestSuccess(data.savingDeposit, data.message))
        })
      }
      else{
                ...
                .then(response => {
                  if(response.ok){
                    response.json().then(data => {
                      dispatch(fetchSavingDepositsSuccess(data.savingDeposits,data.message));
                    })
                  }
                  else{
                    response.json().then(error => {
    ...
    dispatch(fetchSavingDepositRequest());
      ...
      return fetch(apiUrl + savingDepositId)
             .then(response => {console.log(response)
               if(response.ok){
                 response.json().then(data => {
                   dispatch(fetchSavingDepositSuccess(data.savingDeposit[0], data.message));
                 })
               }
               else{
                 response.json().then(error => {
